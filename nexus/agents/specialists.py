...
class SupervisorAgent(BaseAgent):
    ...
    PLAN_SYSTEM = (
        ...
        "- HOSTING tasks → coder uses the start_server tool (one-shot start+verify) or the nohup+curl pattern in a SINGLE shell call; never claim \"hosted\" without a verified HTTP 200 + content marker.\n"
...
class WorkerAgent(BaseAgent):
    ...
    system_prompt = (
        ...
        "HOSTING: use the start_server tool (start + verify in one call) — NEVER plain "
        "foreground run_shell for servers, and never claim a server is up without a "
...
class CoderAgent(BaseAgent):
    ...
    system_prompt = (
        ...
        "- Long-running servers: use the start_server tool (start+verify ONE call, "
        "server stays up) OR one-shot pattern in a single run_shell: "
        "'nohup python3 -m http.server PORT --directory DIR >/dev/null 2>&1 & sleep 1; "
        "curl -s localhost:PORT | head -5' — verify REAL content markers, and NEVER "
        "claim hosting without a verified HTTP 200. A foreground server will block "
        "and time out; never pkill a server you started.\n"
